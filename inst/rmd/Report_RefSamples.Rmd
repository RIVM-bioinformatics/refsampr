...
params:
  input_dir:
    value: "C:/Users/alehd/Documents/Test_refsamp/Juno_results/Juno_results/210425_NB502001_0190_AH7YJ5AFX2_0019/"
  history_data: 
...
if(params$run_date == "x"){
  run_date <- get_run_date(params$input_dir)
} else {
  run_date <- get_run_date(params$run_date)
}
...
refsamp_run_metrics <- merging_by_sample(c("bbtools_report", 
                                           "quast_report", 
                                           "checkm_report"), 
                                         run_date = run_date)
...
if ( file.exists(params$history_data) ) {
  ...
  history_data <- history_data %>%
    ...
    filter(difftime(Run_date, Sys.Date(), units = "days") %>% 
             as.numeric() %>% `>`(-360))
